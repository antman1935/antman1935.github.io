...
CSPL.interpolatePoints = function(xs, ys, min, max, points, outxs, outys)
{
  var ks = [];
  CSPL.getNaturalKs(xs, ys, ks);
  const step_size = (max - min) / points;
  if (step_size <= 0) {
    return;
  }
  for (var i = 0; i < points; ++i) {
    const _x = min + (i * step_size);
    const _y = CSPL.evalSpline(_x, xs, ys, ks);
    outxs.push(_x);
    outys.push(_y);
  }
}
